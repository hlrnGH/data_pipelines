...
- **Jeudi** : Spark ne résout pas le hostname `postgres` depuis son conteneur (bitnamilegacy) malgré le même réseau Docker. Problème DNS interne non résolu. Contournement : passer l'IP directe via `SPOTIFY_POSTGRES_URL`. Fix permanent identifié : `extra_hosts` + IP statique dans docker-compose.
- **Vendredi** : En cours...
